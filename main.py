...
class AppUI(QtGui.QMainWindow):
    ...
    def __init__(self):
        ...
        QtCore.QObject.connect(
            self.stop_selector, QtCore.SIGNAL("selected()"), self.boardselected)
        QtCore.QObject.connect(
            self.from_selector, QtCore.SIGNAL("selected()"), self.stopselected)
        QtCore.QObject.connect(
            self.to_selector, QtCore.SIGNAL("selected()"), self.stopselected)
    ...
    def boardselected(self):
        ...
        trams = self.vast.board(
            self.stop_selector.stopId, time_span=120, departures=4)
        ...
        for i in trams:
            ...
            delta = [(q - self.vast.datetime_obj)
                     .seconds / 60 for q in i.datetime_obj]
            delta.sort()
        ...
        for i in range(4):
            self.stopList.resizeColumnToContents(
    ...
    def stopselected(self):
        ...
        trips = self.vast.trip(
            originId=self.from_selector.stopId, destId=self.to_selector.stopId, datetime_obj=d)
        ...
        for trip in trips:
            ...
            for i in trip.legs:
                ...
                leg.setText(0, '%02d:%02d' %
                            (i.origin.datetime_obj.hour, i.origin.datetime_obj.minute))
                ...
                leg.setText(
                    4, str(i.destination.datetime_obj.strftime('%H:%M')))

                item.addChild(leg)
            item.setText(0, '%02d:%02d' %
                         (trip.legs[0].origin.datetime_obj.hour, trip.legs[0].origin.datetime_obj.minute))
            self.tripList.addTopLevelItem(item)

        self.tripList.expandAll()

        for i in range(4):
            self.tripList.resizeColumnToContents(
                i)  # Must be done in order to avoid  too small columns
